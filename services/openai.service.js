...
const analyzeHeadings = (html) => {
  ...
  if (headings.total > 0) {
    ...
    headings.structure = headings.structure.substring(
      0,
      headings.structure.length - 1
    );
  }
...
const analyzeSEO = (content, keywords, meta) => {
  ...
  if (keywords && keywords.length > 0) {
    const densities = keywords.map((keyword) =>
      calculateKeywordDensity(plainText, keyword)
    );
    seoAnalysis.keywordDensity.byKeyword = keywords.reduce(
      (acc, keyword, index) => {
    ...
    seoAnalysis.keywordDensity.average =
      (
    ...
    if (avgDensity < 0.5) {
      ...
      seoAnalysis.keywordDensity.recommendation =
        "Increase keyword usage (target: 1-3%)";
    } else if (avgDensity > 5) {
      seoAnalysis.keywordDensity.status = "warning";
      seoAnalysis.keywordDensity.recommendation =
        "Keyword stuffing detected. Reduce keyword density.";
    } else {
  ...
  if (readabilityScore < 30) {
    ...
    seoAnalysis.readability.recommendation =
      "Text is very difficult to read. Use shorter sentences and simpler words.";
  } else if (readabilityScore < 60) {
    seoAnalysis.readability.status = "moderate";
    seoAnalysis.readability.recommendation =
      "Text is somewhat difficult to read. Try simplifying your language.";
  } else {
...
async function expandContent(parsedContent, targetWordCount) {
  ...
  console.log(
    `Expanding content from ${wordCount} to ${targetWordCount} words`
  );
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.log(
        `Expansion attempt ${attempt}/${maxAttempts}: Adding ~${wordsToAdd} words`
      );
      ...
      5. IMPORTANT: The final expanded content MUST be at least ${
        targetWordCount * 0.9
      } words 
      ORIGINAL ARTICLE:
      ${expandedContent}
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a content expansion specialist. Your task is to expand articles to meet specific word count targets while maintaining quality and relevance. You MUST add enough content to reach the target word count.",
          },
      ...
      console.log(
        `After expansion attempt ${attempt}: Now at ${expandedWordCount}/${targetWordCount} words`
      );

      // If we've added less than 30 words, something is wrong
      if (expandedWordCount < wordCount + 30 && attempt < maxAttempts) {
        console.warn(
          "Expansion added too few words, trying different approach"
        );
        ...
        if (altWordCount > expandedWordCount) {
          ...
          console.log(
            `Alternative expansion improved word count to ${expandedWordCount}`
          );
        }
      }
    } catch (error) {
      console.warn(
        `Content expansion attempt ${attempt} failed:`,
        error.message
      );
  ...
  if (expandedWordCount < wordCount + 30) {
    console.warn(
      `Warning: Expansion barely increased content length (${wordCount} → ${expandedWordCount})`
    );
  }

  if (expandedWordCount < targetWordCount * 0.8) {
    console.warn(
      `Warning: Final expanded content (${expandedWordCount} words) is still well below target (${targetWordCount} words)`
    );
  } else {
    console.log(
      `Successfully expanded content to ${expandedWordCount} words (${Math.round(
        (expandedWordCount / targetWordCount) * 100
      )}% of target)`
    );
  }
 ...
 * @param {string} [existingContent] - Optional existing content to improve upon
...
async function generateContentSuggestion(
  ...
  try {
    ...
    if (!industry?.trim())
      throw new Error("businessContext.industry is required");
    if (!targetAudience?.trim())
      throw new Error("businessContext.targetAudience is required");
    ...
    // Construct structured prompt based on whether custom prompt or existing content is provided
    let prompt;
    if (existingContent) {
      prompt = `Improve and enhance the following content about "${topic.trim()}" for a ${industry.trim()} business targeting ${targetAudience.trim()}.
      EXISTING CONTENT:
      ${existingContent}

      TITLE: [An engaging title under 60 characters]
      The improved content must follow this EXACT format:
      KEYWORDS: [5-7 relevant keywords separated by commas]
      META: [A compelling meta description under 155 characters]
      CONTENT: [The improved main article content with proper HTML formatting using <h2>, <h3>, <p> tags]

      IMPORTANT REQUIREMENTS:
      1. The CONTENT section should be AT LEAST ${Math.floor(
        wordCount * 0.5
      )} words. Target ${wordCount} words if possible.
      ...
      5. Maintain the core message and key points from the existing content while improving its structure and SEO.
      ${
        normalizedKeywords.length > 0
          ? `6. Include these keywords naturally in the content: ${normalizedKeywords.join(
              ", "
            )}`
          : ""
      }
      Improve the content now, focusing on quality, structure, and SEO optimization.`;
    } else {
      // Base prompt structure that will be used for both custom and default cases
      const basePromptStructure = `Write a well-structured, SEO-optimized article on "${topic.trim()}" for a ${industry.trim()} business targeting ${targetAudience.trim()}.
      ...
      ${
        normalizedKeywords.length > 0
          ? `5. Include these keywords naturally in the content: ${normalizedKeywords.join(
              ", "
            )}`
          : ""
      }`;
      ...
      if (customPrompt) {
        // Combine custom prompt with the required structure
        prompt = `${basePromptStructure}
        ${customPrompt}
        ADDITIONAL CUSTOM REQUIREMENTS:
        Write the article now, focusing on quality and proper structure while incorporating the custom requirements.`;
      } else {
        // Use the base prompt structure with default instructions
        prompt = `${basePromptStructure}
      ...
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a professional SEO content writer. Generate highly optimized content with structured formatting and engaging readability. Always include TITLE, META, KEYWORDS, and CONTENT sections.",
              },
          ...
          if (!validateResponse(responseContent)) {
            console.warn(
              `Invalid response structure on attempt ${attempt}. Missing required sections.`
            );
            if (attempt === retries) {
              throw new Error(
                "Failed to generate properly structured content after multiple attempts"
              );
            }
            await delay(2000);
            continue;
          ...
          if (parsedContent.wordCount < wordCount * 0.25) {
            console.warn(
              `Word count too low on attempt ${attempt}: Requested ${wordCount}, got ${parsedContent.wordCount}.`
            );
            if (attempt === retries) {
    ...
    console.log(
      ...
      }/${wordCount} words (${Math.round(
        (parsedContent.wordCount / wordCount) * 100
      )}% of target)`
    );
...
- Word count: ${finalContent.wordCount}/${wordCount} (${Math.round(
      (finalContent.wordCount / wordCount) * 100
    )}% of target)
